...
#BiocManager::install(c("GenomicDistributions",   "GenomicRanges" ))
...
BLAST.df <- read.delim( "./Alignments/BLAST/BLAST_NUPT.tsv" , na.strings=c("","NA"), header = FALSE, sep = "\t", quote = "", dec = "." )
...
for( i in names(BLAST.alignment.coordinates.plastid.genome) ) {
  ...
  files.NUPTs.plastid.genome[i] <- paste0("./Spatial_distribution_analyses/Spatial_distribution_plastid_genome/BED.NUPTs.plastid.genome/" , i, ".NUPTs.Plastid.genome.bed"   )
...
We used the rule of the Interquartile Range (IQR) to detect large outliers in the the distribution of NUPT counts across the plastid genomes which we classified as hotspots. Non-donating regions (Null counts of NUPTs = 0 detected NUPTs) were labelled as coldspots.
...
setwd( "./Spatial_distribution_analyses/Spatial_distribution_plastid_genome/BED.density.NUPTs.plastid.genome.regions.1" )  ## load directory

files <- list.files(path =  "./Spatial_distribution_analyses/Spatial_distribution_plastid_genome/BED.density.NUPTs.plastid.genome.regions.1" , pattern = "*.bed")  # BED file (alignments in tabular format)
...
# Create ID column containing the name of the species
...
Two non-parametric correlation tests were implemented: Kendall´s and Spearman´s.
...
```{r plot_1, fig.width=18, fig.height=20}
...
Plastid_genome_regions  <-  read_excel( 'Genomes_NUPT_paper/Properties_genomes_NUPTs.xlsx'  , sheet = "Plastid_genomes_regions_"  ) 
...
### In some species like Medicago truncatula (Mtr) the IRs are absent.
...
```{r plot_2, fig.width=18, fig.height=20}
...
BLAST <- read.delim( "BLAST_NUPT.tsv" , na.strings=c("","NA"), header = FALSE, sep = "\t", quote = "", dec = "." )    ## Load BLAST files that were previously merged and tagged
...
# Load text file containing sizes of individual chromosomes and scaffolds   
  ...
  setwd(  "NuclearGenomes_Chromosomes_Sizes"  )
  
  files <- list.files(path =  "NuclearGenomes_Chromosomes_Sizes"  , pattern = "size")  
  ...
  setwd(  "TE_classification"   )
  
  files <- list.files(path =  'TE_classification'  , pattern = "bed")
  ...
  for( l in 1:length(Partitions) )  {
    ...
    Partitions[l]  <- mclapply(Partitions[l],  mc.cores = 12  , function(x) {
      ...
      z <- GenometriCorrelation(   NUPTs_ , Partition , chromosomes.length =  scaffold.sizes.vector_ , awhole.only = T,   # Perform test only on the genome as a whole and not each individual chromosome
                                   permut.number = 1000, 
...
load( './Spatial_distribution_analyses/Spatial.analysis.NUPTs.NoDupl.TE.Order.RData'   )


Spatial.analysis.NUPTs.TE.Order  <- Filter(function(x) !is.data.frame(x), Spatial.analysis.NUPTs.TE.Order)


# Extract list "awhole" (Corresponds to the genome, i.e., the chromosomes as a whole)
...
# Merge list into dataframe. As a preliminary step, to distinguish the species, we create the column "Species"
...
##### 4.2.1. Test based on counts of overlapping bp (Chi-squared test) 
...
## We customized the function used by the the package to calculate the overlap with the aim of generating more data
...
# We have to merge overlapping genomic features of the same class. This operation  eliminates redundant overlaps between two diffent classes of genomic features 
...
# NUPTs  
  ...
  NUPTs <-  GenomicRanges::reduce(NUPTs)  # Merge overlapping genomic intervals
 ...
 # RNA genes 
  ...
  setwd(  "./RNA_genes/Nuclear/Output_RFAM"   )
  
  files <- list.files(path =  "./RNA_genes/Nuclear/Output_RFAM"  , pattern = "tsv")
  ...
  Overlap.calc[Overlap.calc == -Inf] <- 0  ##  -Inf values indicate zero overlapping bp between two classes of genomic features. We have to convert them to 0.
...
setwd(  "./NuclearGenomes/NuclearGenomes_Chromosomes_Sizes"  )

    files <- list.files(path =  "./NuclearGenomes/NuclearGenomes_Chromosomes_Sizes"  , pattern = "size") 
...
names(scaffold.sizes.vector) <-  scaffold.sizes$V1
  ...
  save( Spatial.analysis.NUPTs.RNA ,  file = "Spatial.analysis.NUPTs.RNA.RData" ) ## Save results
...
load(  "./Spatial_distribution_analyses/Spatial.analysis.NUPTs.RNA.RData"  )
